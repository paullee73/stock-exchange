...
def SearchAuthentication(request, hashed):
    if(request.method == "GET"):
        try:
            user = Authenticator.objects.get(authenticator=hashed)
            data = {}
...
def ViewOrUpdateStock(request, uniqueID):
    if(request.method == "GET"):
        ...
        user = stock.user
        price = stock.price
        return JsonResponse({'ticker_symbol': ticker, 'owner': user.id, 'price': price})
